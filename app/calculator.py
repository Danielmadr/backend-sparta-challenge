def calculate_management_fee_per_shareholder(data):
    ...
    shareholder_quota_data = data["cotas"]

    num_shareholders = len(shareholder_quota_data[0]["quantidades"])
    fees_per_shareholder = [0.0] * num_shareholders

    for shareholder_idx in range(num_shareholders):  # Loop sobre cada cotista
        accumulated_fee_for_shareholder = 0.0
        for daily_data in shareholder_quota_data:  # Loop sobre cada dia
            quota_value = daily_data["valor"]
            num_quotas = daily_data["quantidades"][shareholder_idx]
            daily_fee = (num_quotas * quota_value * management_fee_rate) / BUSINESS_DAYS_PER_YEAR
